...
use crate::ctx::{BaseCtx, ProfileCtx};
...
fn render_profile(
    state: &AppState,
    user: &gtid_shared::entities::user::User,
    csrf_token: &str,
...
pub async fn password_submit(
    ...
    if let Err(msg) = crate::handlers::validate_password(&form.new_password, t) {
        let rendered = render_profile(
            &state,
            &user,
            &form.csrf_token,
...
pub async fn email_change_submit(
    ...
    let new_email = crate::handlers::normalize_email(&form.new_email);
    ...
    let (subject, body_html) = crate::handlers::render_email_template(
        template.as_ref(),
        name,
        &link,
