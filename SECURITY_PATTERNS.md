...
- **Proxy IP spoofing:** `X-Forwarded-For` is only evaluated when `TRUSTED_PROXIES=true` — by default the direct socket address is used. Prevents IP spoofing behind untrusted proxies
